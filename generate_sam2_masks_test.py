"""
generate_sam2_masks_test.py
===========================
Phase -1 (Blocker): Generate and save SAM2 binary masks for the SECOND test set.

Current tokens_T1_test/*.pt only contain {tokens, centroids, areas} — no masks.
This script re-runs SAM2 AutomaticMaskGenerator on each image and saves masks
separately as .npz (compressed) without touching the existing token files.

Output:
    SECOND/sam2_masks_T1_test/{stem}.npz  → {"masks": (N,512,512) bool,
                                               "scores": (N,) float32,
                                               "bboxes": (N,4) int32}
    SECOND/sam2_masks_T2_test/{stem}.npz

Usage:
    python generate_sam2_masks_test.py
    python generate_sam2_masks_test.py --resume   # skip existing files
"""
...
OUT_T1_DIR = DATA_ROOT / "sam2_masks_T1_test"
OUT_T2_DIR = DATA_ROOT / "sam2_masks_T2_test"

SAM2_CKPT   = os.path.join(SAM2_REPO, "checkpoints", "sam2.1_hiera_large.pt")
SAM2_CONFIG = "configs/sam2.1/sam2.1_hiera_l.yaml"

# Same defaults as tokenize_regions.py for consistency
POINTS_PER_SIDE  = 32
...
def process_masks(raw_masks: list) -> dict:
    ...
    if not raw_masks:
        # Fallback: one all-true mask so downstream doesn't break
...
def main():
    ...
    log.info(f"Found {len(stems)} test stems")
    assert len(stems) == 1694, f"Expected 1694 stems, got {len(stems)}"

    amg = build_amg(device)

    skipped = errors = 0

    for stem in tqdm(stems, desc="Generating SAM2 masks"):
    ...
    log.info("✅ Phase -1 complete. SAM2 masks saved.")
